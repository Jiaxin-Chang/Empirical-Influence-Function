...
class TTAVBundleRequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_llm_train_retrieve(self):
        """LLM proposes corpus search expressions for whole-test FIM + gold."""
        _hydrate_eif_env()
        content_length = int(self.headers.get("Content-Length", "0"))
        raw_body = self.rfile.read(content_length)
        try:
        ...
        try:
            from src.llm_train_retrieval import retrieve_llm_train_samples

            result = retrieve_llm_train_samples(
    ...
    def _handle_llm_corpus_search(self):
        ...
        expression = str(req.get("expression") or req.get("goldExpr") or req.get("gold_expr") or "").strip()
        if not expression:
