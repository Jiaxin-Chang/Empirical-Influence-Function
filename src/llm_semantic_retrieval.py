...
def build_valid_semantic_messages(
    *,
    ...
    system = (
        ...
        "Relations are the exception: if an operation already names the missing act, "
        "the relation must still state that act as a link. Do not thin the relation "
        "into a generic connection just to avoid repeating the operation.\n"
        ...
        "- If the prediction and the gold already share the same object, do not "
        "relate that object to a generic result or to \"a setting on it\". "
        "Relate the treatment the prediction used to the treatment the gold uses. "
        "Raw rows read from a reader → records filled into the destination "
        "structure. A skipped option is the behavior it enables "
