...
def build_valid_semantic_messages(
    *,
    ...
    system = (
        ...
        "A different way of doing the same job still shares the job. "
        "Do not take this branch just because the gold uses another implementation.\n"
        "- If the prediction already matches the gold, there is no residual. "
        "Describe the gold span as a whole.\n\n"
        "Use exactly four fields. They form one closed loop and should not restate each other.\n"
        "A relation may repeat the missing act already named by an operation. "
        "Do not shorten that link into \"shared object → a result\" or "
        "\"shared object → a setting\" just to avoid the repeat.\n"
        ...
        "- If the prediction and the gold already share the same object, this rule "
        "overrides the consume → produce shape. The two ends are the two treatments, "
        "not the object and a result, and not the object and a setting. "
        "Source is the treatment the prediction used. Target is the treatment the gold uses. "
        "Raw rows read from a reader → records filled into the destination structure. "
        "A skipped option is the behavior it enables "
        "(a trailing field separator is accepted), not the field's name, and not "
        "every configuration of that object. "
        "This rule is only for a different use of the same object. "
        "It also applies when the prediction already does the same job a different way: "
        "that is still two treatments, not a gold-only span. "
        "Fields split on spaces → the message returned by the library parser. "
        "A later cleanup of an object the prediction already created stays the lifetime link below.\n"
        "- Otherwise the typical shape is: what the missing piece consumes → what it produces, and "
        ...
        "When both sides already share an object, name the treatment difference, "
        "not \"configure that object\".\n"
        ...
        "Do not use a phrase that also fits every configuration of the shared object.\n"
        "Operations: 1-4 semantic acts of the residual only, in order. "
        "Do not list acts the prediction already performed correctly. "
        "Do not write \"set <Field> to true\" or \"configure the reader\"; "
        "write the behavior (a trailing field separator is accepted) "
        "or the treatment (fill records into the destination structure).\n"
    ...
    user = (
        ...
        "If the prediction is empty or unrelated, describe the whole gold span. "
        "If the prediction already does the same job a different way, the two ends are those two treatments, "
        "not the shared input and a generic result. "
        "Fields split on spaces → the message returned by the library parser. "
        "Do not emit raw string → parsed message for that case.\n"
        "Return only the required JSON object."
    )
    return [
        {"role": "system", "content": system},
