#!/usr/bin/env python3
"""Gold substring search → MID rewrite → tree-sitter-only GraphSignal annotate.

For each test row (``prompt`` + gold in ``label``/``response``; ignore ``predict``):

  1. Clean gold (strip ``` fences / thinking noise)
  2. Scan train corpus for a row containing that gold
  3. MID-rewrite the hit so dig teaches the gold
  4. Annotate with GraphSignal **tree-sitter only** (no LLM)
  5. Write ``input_ids`` / ``label`` / ``attention_edges`` (× copies)

Does **not** need vLLM. Needs tokenizer + ``tree-sitter`` / ``tree-sitter-cpp``.

Example (C++ eval JSONL like your sample)::

    python -m src.fast_gold_treesitter \\
      -i /path/to/eval_or_test.jsonl \\
      --corpus /path/to/cpp_train_fixed.jsonl \\
      -o /path/to/cpp_gold_ts_ids.jsonl \\
      --tokenizer /path/to/Qwen3-8B \\
      --language cpp --copies 1
"""
...
DEFAULT_OUTPUT = (
    "/mnt/md124/jiaxin/Empirical-Influence-Function/cpp_fast_gold_treesitter.jsonl"
)
...
def _strip_markdown_fence(text: str) -> str:
    """Unwrap a single outer ```lang ... ``` fence if present."""
    t = (text or "").strip()
    m = _FENCE_RE.match(t)
    if m:
        return m.group(1).strip()
    # Also handle fence without trailing newline before closing ```
    if t.startswith("```"):
        nl = t.find("\n")
        if nl > 0:
            body = t[nl + 1 :]
...
def _test_fim_gold(row: dict[str, Any]) -> tuple[str, str]:
    """Prefer explicit gold fields; never use ``predict`` as training gold."""
...
def _rewrite_train(prompt: str, response: str, *, test_gold: str):
    from src.fast_gold_to_continue import _rewrite_train as _rw

    return _rw(prompt, response, test_gold=test_gold)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "test gold → corpus search → MID rewrite → tree-sitter-only annotate"
        ),
    )
    p.add_argument("-i", "--input", default=DEFAULT_TEST, help="test/eval JSONL")
    p.add_argument("-o", "--output", default=DEFAULT_OUTPUT, help="output JSONL")
    p.add_argument("--corpus", default=DEFAULT_CORPUS, help="train corpus JSONL")
    ...
    p.add_argument("--max-tests", type=int, default=0, help="0 = no limit")
    p.add_argument("--max-scan", type=int, default=0, help="0 = full corpus")
    ...
    p.add_argument("--tokenizer", default="", help="tokenizer / base model dir")
    ...
    p.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="write input_ids+label+attention_edges only (default true)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    state_path = (
        ...
        if args.state
        else input_path.with_suffix(input_path.suffix + ".fast_gold_ts_state.json")
    )
    state: dict[str, Any] = {
        "ok_tests": [],
        "skipped": [],
    ...
    from src.fast_gold_to_continue import _iter_corpus_hits, _row_prompt_response

    print(f"[fast-gold-ts] input={input_path}", flush=True)
    print(f"[fast-gold-ts] corpus={corpus_path}", flush=True)
    print(f"[fast-gold-ts] output={out_path}", flush=True)
    print(
        f"[fast-gold-ts] tokenizer={tok_path} copies={args.copies} "
        f"language={args.language} use_llm=False",
        flush=True,
    )
    print("[fast-gold-ts] loading tokenizer…", flush=True)
    tokenizer = _load_tokenizer(tok_path)
    print("[fast-gold-ts] tokenizer ready", flush=True)
